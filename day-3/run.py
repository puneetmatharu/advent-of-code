...
def part1(fpath: Path):
    return sum([v for (i, s, e, v) in [[i, *m.span(), int(m.group(0))] for (i, line) in enumerate(load(fpath)) for m in re.finditer(r"(\d+)", line)] if (grid := np.pad(np.array([list(l) for l in load(fpath)], dtype=str), 1, constant_values=".")).size > 0 if not all([c in "0123456789." for c in [*grid[i, s:e + 2], *grid[i + 1, [s, e + 1]], *grid[i + 2, s:e + 2]]])])
